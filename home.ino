...
void setup() {
  Serial.begin(9600); // Set baud rate for communication with phone

  // Set relay pins as outputs and switch them OFF initially
...
void loop() {
  while (Serial.available()) {     // Check if data is available
    ...
    inputs += c;                   // Append character to input string
  }
  ...
  if (inputs.length() > 0) {
    ...
    else if (inputs == "E") digitalWrite(relay5, LOW);
    else if (inputs == "e") digitalWrite(relay5, HIGH);

    else if (inputs == "F") digitalWrite(relay6, LOW);
    else if (inputs == "f") digitalWrite(relay6, HIGH);

    else if (inputs == "G") digitalWrite(relay7, LOW);
    else if (inputs == "g") digitalWrite(relay7, HIGH);

    else if (inputs == "H") digitalWrite(relay8, LOW);
    else if (inputs == "h") digitalWrite(relay8, HIGH);

    inputs = ""; // Clear input after processing
  }
}
